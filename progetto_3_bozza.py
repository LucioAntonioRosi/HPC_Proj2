...
# n_rows non è J!!
def S_operator(J, x, Bj_list, Sj_list, Cj_list, Tj_list, displacement = 0, n_rows = nx):
    """
    Input:
        nx, ny: number of points in the x and y directions
        Lx, Ly: lenghts of the domain in the x and y direction
...
def create_matrices(nx, ny, Lx, Ly, sp, k, J, displacement = 0, n_rows = J):
    ...
    print(f"displacement: {displacement}, n_rows: {n_rows}")
...
def par_fixed_point(comm, nx, ny, Lx, Ly, sp, k, J, p0, w, tol = 1e-12, iter_max = 10000):
    rank = comm.Get_rank()
    size = comm.Get_size()
    ...
    rows_per_process = [(J // size + (1 if i < J % size else 0)) for i in range(size)]
    displacements = [sum(rows_per_process[:i]) for i in range(size)]
    
    print(f"rank = {rank}: rows per process: {rows_per_process}")
    
    # They are all local matrices
    _, _, _, Tj_list, Bj_list, Cj_list, Sj_list, _, bj_list = create_matrices(nx, ny, Lx, Ly, sp, k, J, 0, rows_per_process[rank])

    g_local = g_vector(rows_per_process[rank], Sj_list, Cj_list, Bj_list, bj_list)
    g_global = np.zeros_like(g_local)
    comm.Allreduce(g_local, g_global, op=MPI.SUM)

    p_next = np.zeros_like(g_global)
    p_0 = np.copy(p0)
    
    residual = 1
    while iterations < iter_max and residual > tol:
        y_local = S_operator(rows_per_process[rank], p_0, Bj_list, Sj_list, Cj_list, Tj_list)

        y_global = np.zeros_like(y_local)
        comm.Allreduce(y_local, y_global, op=MPI.SUM)

        PiSp0 = Pi_operator(nx, J, y_global)

        p_next = None
        if rank == 0:
            p_next = (1 - w) * p_0 - w * PiSp0 + w * g_global
            residual = np.linalg.norm(p_0 + Pi_operator(nx, J, S_operator(J, p_0, Bj_list, Sj_list, Cj_list, Tj_list)) - g_global, ord=2)

        p_next = comm.bcast(p_next, root = 0)
    ...
    final_solution = None
    if rank == 0:
        final_solution = np.copy(p_next)

    return final_solution, iterations, residuals
...
def MyGmres(nx, ny, Lx, Ly, sp, k, J, tol = 1e-12):
    ...
    #y, _ = spla.gmres(A, g, tol = tol, callback=callback2, callback_type='pr_norm')
...
def main():
    ...
    y_fixed, iter, err = par_fixed_point(comm, nx, ny, Lx, Ly, sp, k, J, x1, 0.5)
